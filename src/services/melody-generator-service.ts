...
export class MelodyGeneratorService implements IMelodyGeneratorService {
  ...
  getMelody(
    ...
    const allowedNotes: NoteLetter[] = noteLetterOptionalPairs.map(
    ...
    const fretsOfNotesOnAllStrings: (number | null)[][] =
      this.getsFretsOfNotesOnAllStrings(allowedNotes, tuning, minFret, maxFret);
    ...
    let lastNote: INote = this.musicTheoryService.getNoteFromFret(
      tuning[melodyNote.stringIndex],
      melodyNote.fret
    );
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const distancePositive: number = Math.round(
        this.nextExponential(1 / averageNoteJump)
      );
      ...
      const indexOfLastNoteLetter: number = allowedNotesSorted.findIndex(
        (noteLetter) => noteLetter === lastNote.letter
      );
      const newNoteLetterIndex: number =
        indexOfLastNoteLetter + Math.abs(distance);
      const octaveFactor: number =
        Math.floor(newNoteLetterIndex / allowedNotesSorted.length) *
        (distance < 0 ? -1 : 1);
      const newNoteLetterIndexCorrected: number =
        newNoteLetterIndex - allowedNotesSorted.length * Math.abs(octaveFactor);
      const newOctave: number = lastNote.octave + octaveFactor;
      const newNote: INote = {
        letter: allowedNotesSorted[newNoteLetterIndexCorrected],
        octave: newOctave,
      };

      const newMelodyChord: (number | null)[] =
        this.musicTheoryService.getNullChord(tuning.length);

      if (
        this.musicTheoryService.getNoteValue(newNote) ===
        this.musicTheoryService.getNoteValue(lastNote)
      ) {
      ...
      while (!found && fretsOfNotesOnAllStringsCopy.length > 0) {
        const randomStringIndex = this.getRandomIntInclusive(
          0,
          fretsOfNotesOnAllStringsCopy.length - 1
        );
        ...
        for (let k = 0; k < fretsOnString.length; k++) {
          if (
            this.musicTheoryService.getNoteValueFromFret(
              tuning[randomStringIndex],
              fretsOnString[k]!
            ) === this.musicTheoryService.getNoteValue(newNote) &&
            (lastFret === null || Math.abs(fretsOnString[k]! - lastFret) <= 4)
          ) {
